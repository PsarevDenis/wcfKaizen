(function ($) {
    ...
    $.getJSON("http://localhost:64378/Service1.svc/GetClassifier", function (data) {
        ...
        $.each(data['GetClassifierResult'], function (key, val) {
            var classifier = { 'Id': val['Id'], 'Name': val['Name'] };
            classifierList.push(classifier);
        });
    });

    $.fn.orgChart.defaults = {
        data: [{ id: 1, name: 'Причина', classifier: '1', prioritizing: '10', parent: 0}],
        showControls: false,
        allowEdit: false,
        onAddNode: null,
        onDeleteNode: null,
    ...
    function OrgChart($container, opts){
        ...
        this.startEdit = function (id, element) {
            ...
            if (element === 'h3') {
                ...
                var inputElement1 = $('<select class="org-input"><option value=' + classifierList[0].Id + '>' + classifierList[0].Name + '</option><option value=' + classifierList[1].Id + '>' + classifierList[1].Name + '</option></select>');
                ...
                var commitChange1 = function () {
                    ...
                    jQuery.each(classifierList, function () {
                        if (String(this.Id) === nodes[id].data.classifier) {
                            className = this.Name;
                        }
                    });

                    var h3Element = $('<h3>' + className + '</h3>');
        ...
        this.newNode = function (parentId) {
            ...
            self.addNode({ id: nextId, name: 'Новая причина', classifier: '1', prioritizing: '0', parent: parentId });
        };
    ...
    function Node(data){
        ...
        this.formatNode = function (opts) {
            ...
            if (typeof data.classifier !== 'undefined') {
                ...
                jQuery.each(classifierList, function (index, value) {
                    if (String(value.Id) === String(data.classifier)) {
                        className = value.Name;
                    }
                });

                classifierString = '<h3>' + className + '</h3>';
            }
